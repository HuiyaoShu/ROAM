...
class Graph:
    def __init__(self, name=""):
        ...
        self.grad_apply = None
        self.grad_nodes = None
    ...
    def canonicalize(self, del_params=True):
        if self.canonical:
            return
        ...
        if del_params:
            ...
            for node in self.nodes.values():
                if node.is_stateful() or "buffers" in node.name:
                    nodes.add(node)
    ...
    def sink_reachability(self, node):
        reachability = set()
        ...
        while len(BFS) > 0:
            ...
            reachability.add(current)
            for e in current.fanin:
                source = e.source
                if source not in BFS:
                    BFS.append(source)
        
        return reachability

    def source_reachability(self, node):
        reachability = set()
        ...
        while len(BFS) > 0:
            current = BFS.pop(0)
            if current in traverse:
                continue

            traverse.add(current)

            for e in current.fanout:
                for sink in e.sinks:
                    reachability.add(sink)
                    if sink not in BFS:
                        BFS.append(sink)
        
        return reachability

    def post_process_mobilenet(self):
        ...
        for i in range(35):
            ...
            clone_node = self.find_node(clone)
            sink = clone_node.fanout[0].sinks[0]
            for e in sink.fanin:
                src = e.source
                if fwd_levels[src] + 1 == fwd_levels[sink]:
                    self.add_edge(
                        [src],
                        [clone_node],
                        size=0,
                        name=clone_node.name + "_overlap_branch_forced_late",
                    )
                    break
    ...
    def post_process_efficientnet(self):
        fwd_levels = self.build_levelization()
        bwd_levels = self.build_reverse_levelization()

        sum_1 = self.find_node("sum_1")
        ...
        for i in range(50):
            ...
            while len(BFS) != 0:
                ...
                # find candidate.
                candidate = self.constrain_branch_node(node, fwd_levels, bwd_levels)
                
                # print("[***] Candidate for {} is: {}".format(node.name, candidate.name))
        self.canonical = False
        self.canonicalize()

    def post_process_LLM(self):
        sum_1 = self.nodes["sum_1"]
        fwd_levels = self.build_levelization()
        bwd_levels = self.build_reverse_levelization()
        sinks = self.sink_reachability(sum_1)
        sources = self.source_reachability(sum_1)
        branch_nodes = []
        for n in self.nodes.values():
            if n in sinks:
                continue
        ...
        for n in branch_nodes:
            self.constrain_branch_node(n, fwd_levels, bwd_levels)
    ...
    def constrain_weight_updates(self):
        ...
        '''
        for t, ns in self.fwd_level_node.items():
            if len(ns) == 1:
                for n in ns:
                    bwd_lvl = bwd_levels[n]
                    if len(self.bwd_level_node[bwd_lvl]) == 1:
                        print("[***] Find keypoint: {}".format(n))
    ...
    def _sort(self, zero_fanouts, fwd_levels):
        length = len(zero_fanouts)
        for i in range(1, length):
            for j in range(0, length - i):
                node1 = zero_fanouts[j]
                node2 = zero_fanouts[j + 1]
                if fwd_levels[node1] > fwd_levels[node2]:
                    zero_fanouts[j] = node2
                    zero_fanouts[j+1] = node1
                elif fwd_levels[node1] == fwd_levels[node2]:
                    addcdiv__ = self.nodes["addcdiv__opti"]
                    flag1 = False
                    for e in node1.fanin:
                        if e.source == addcdiv__:
                            flag1 = True

                    flag2 = False
                    for e in node2.fanin:
                        if e.source == addcdiv__:
                            flag2 = True
                    ...
                    size1, size2 = 0, 0
                    for e in node1.fanin:
                        size1 = max(size1, e.size)
                    for e in node2.fanin:
                        size2 = max(size2, e.size)
                    if size1 > size2:
                        zero_fanouts[j] = node2
                        zero_fanouts[j+1] = node1
    ...
    def build_apply_grad(self):
        # apply zero fanout node => gradient node
        self.apply_grad = {}
        # gradient node => apply node
        self.grad_apply = defaultdict(lambda: [])
        # gradient nodes
        self.grad_nodes = set()

        for node in self.nodes.values():
            if "_opti" in node.name and len(node.fanout) == 0:
                BFS = [node]
                traverse = set()
                flag = False
                while len(BFS) > 0 and not flag:
                    n = BFS.pop(0)
                    if n in traverse:
                        continue
                    traverse.add(n)
                    ...
                    for e in n.fanin:
                        src = e.source
                        if "opti" in src.name:
                            if src not in traverse and src not in BFS:
                                BFS.append(src)
                            continue
                        
                        for e in src.fanout:
                            for sink in e.sinks:
                                if "_opti" in sink.name:
                                    flag = True
                        if flag:
                            # Get gradient node.
                            self.grad_nodes.add(src)
                            self.apply_grad[node] = src
                            self.grad_apply[src].append(node)
                            break

        return self.grad_nodes, self.apply_grad


    def constrain_weight_updates_early(self):
        reachable_set = self.build_reachable_set()
        fwd_levels = self.build_levelization()
        bwd_levels = self.build_reverse_levelization()
        longest_path = self.longest_path_length()

        # import pdb;pdb.set_trace()
        if not self.apply_grad:
            self.build_apply_grad()
        
        zero_fanouts = []
        for node in self.nodes.values():
            if len(node.fanout) > 0:
                continue
            zero_fanouts.append(node)
        
        zero_fanouts = self._sort(zero_fanouts, fwd_levels)

        # print("Zero_fanouts node: {}".format(zero_fanouts))
        
        last_apply = self.nodes["addcdiv__opti"]
        process = set()
        if len(last_apply.fanout) > 2:
            # import pdb;pdb.set_trace()
            for e in last_apply.fanout:
                if e.size != 0:
                    continue
                
                sink = e.sinks[0]
                target = None
                for i in range(len(zero_fanouts)):
                    if self.apply_grad[zero_fanouts[i]] == sink:
                        if i < len(zero_fanouts) - 1:
                            target = zero_fanouts[i + 1]
                
                if target:
                    for a, g in self.apply_grad.items():
                        if a == target:
                            target = g
                    ...
                    self.add_edge(
                        ...
                        [target],
                        size=0,
                        name=sink.name + "_forced_early",
                    )
                    process.add(sink)
                

        # last_grad = self.apply_grad[zero_fanouts[-1]]
        # bwd_levels[last_grad] += 1
        
        for n_zero_fanout in zero_fanouts:
            if n_zero_fanout in process:
                continue

            if n_zero_fanout == zero_fanouts[-1]:
                continue

            if len(n_zero_fanout.fanout) > 0:
                continue

            # if n_zero_fanout.name == "addcdiv__13_opti":
            #     import pdb;pdb.set_trace()

            best_candidate_node = None
            if n_zero_fanout in self.apply_grad:
                grad_node = self.apply_grad[n_zero_fanout]
                min_fwd_level = fwd_levels[grad_node]
            else:
                grad_node = n_zero_fanout
                min_fwd_level = fwd_levels[n_zero_fanout]
            
            max_bwd_level = 0
            candidate_sources = set([n_zero_fanout])
            traverse = set()
            while not best_candidate_node and len(candidate_sources) > 0:
                candidate_sources = self._generate_candidate_sources(candidate_sources)
                for source in candidate_sources:
                    ...
                    while len(BFS) > 0:
                        ...
                        for e in node.fanout:
                            for sink in e.sinks:
                                flag = 1
                                if bwd_levels[sink] <= max_bwd_level:
                                    continue
                                
                                if n_zero_fanout not in reachable_set[sink] \
                                    and fwd_levels[sink] >= min_fwd_level:
                                    max_bwd_level = bwd_levels[sink]
                                    best_candidate_node = sink
                                    flag = 0
                                
                                if flag and sink not in traverse and sink not in BFS:
                                    BFS.append(sink)
                ...
                if best_candidate_node:
                    self.add_edge(
                        ...
                        [best_candidate_node],
                        size=0,
                        name=n_zero_fanout.name + "_forced_early",
                    )
                
                    # Update reachable set.
                    to_update = reachable_set[best_candidate_node]
                    for node in to_update:
                        if node in self.apply_grad:
                            g_node = self.apply_grad[node]
                            g_node_lvl = fwd_levels[g_node]
                        else:
                            g_node = node
                            g_node_lvl = fwd_levels[node]
                        ...
                        while len(BFS) > 0:
                            ...
                            if fwd_levels[n] < g_node_lvl:
                                continue
                            else:
                                reachable_set[n].add(node)
                                if n not in BFS and n not in traverse_1:
                                    BFS.append(n)
    ...
    def post_process(self):
        if "mobilenet" in self.name:
            self.post_process_mobilenet()
        elif "efficientnet" in self.name:
            self.post_process_efficientnet()
        elif "gpt" in self.name:
            self.post_process_LLM()

    def constrain_single_weight(self, n_zero_fanout):
        reachable_set = self.build_reachable_set()
        fwd_levels = self.build_levelization()
        bwd_levels = self.build_reverse_levelization()
        longest_path = self.longest_path_length()
        ...
        if len(n_zero_fanout.fanout) > 0:
            print("{} is node zero-fanout node.".format(n_zero_fanout))
            return
    ...
    def constrain_branch_node(self, n, fwd_levels, bwd_levels):
        # print("CONSTRAINING BRANCH NODE: {}".format(n))
        min_fwd_level = fwd_levels[n]
        max_bwd_level = 0
        best_candidate_node = None
        candidate_sources = set([n])
        ...
        while not best_candidate_node and len(candidate_sources) > 0:
            ...
            if best_candidate_node:
                ...
                self.add_edge(
                    ...
                    [best_candidate_node],
                    size=0,
                    name=n.name + "_overlap_branch_forced_early",
                )
        
        return best_candidate_node

    # @yyp
    def constrain_adam_zero_fanin(self):        
        fwd_levels = self.build_levelization()
        bwd_levels = self.build_reverse_levelization()
        ...
        for n in self.nodes.values():
            ...
            min_bwd_level = bwd_levels[n]
            max_fwd_level = 0
            best_candidate_node = None
            candidate_sinks = set([n])
            cache = {}
            while not best_candidate_node and len(candidate_sinks) > 0:
                # generate candidate_sinks.
                candidate_sinks = self._generate_candidate_sinks(candidate_sinks)

                for sink in candidate_sinks:
                    candidate, level = self._find_candidate_zero_fanin(
                        sink, fwd_levels, bwd_levels, min_bwd_level, cache
                    )

                    if level > max_fwd_level:
                        max_fwd_level = level
                        best_candidate_node = candidate
    ...
    # Find anchor node for each sink.
    def _find_candidate_zero_fanin(self, node, fwd_levels, bwd_levels, min_bwd_level, cache):
        if node in cache:
            return cache[node]
        ...
        for fi in node.fanin:
            for source in fi.sources:
                if fwd_levels[source] <= max_fwd_level:
                    continue
                
                if bwd_levels[source] <= min_bwd_level:
                    candidate, level = self._find_candidate_zero_fanin(
                        source, fwd_levels, bwd_levels, min_bwd_level, cache
                    )
                    if level > max_fwd_level:
                        max_fwd_level = level
                        best_candidate = candidate
                else:
                    max_fwd_level = fwd_levels[source]   
                    best_candidate = source
        
        cache[node] = (best_candidate, max_fwd_level)
        return (best_candidate, max_fwd_level)
    
    def adjust_apply_position(self, LIMIT):
        fwd_levels = self.build_levelization()
        bwd_levels = self.build_reverse_levelization()

        # Get memory of activations per timestep.
        sum_1 = self.find_node("sum_1")
        middle = fwd_levels[sum_1]
        activations = []
        mem_per_timestep = defaultdict(lambda: 0)
        for e in self.edges.values():
            # Filter edges created by zero-fanin nodes in apply branch.
            if "_opti" in e.name:
                continue
            ...
            tmp_max_fwd_lvl = -1
            for s in e.sinks:
                if fwd_levels[s] > tmp_max_fwd_lvl:
                    sink = s
                    tmp_max_fwd_lvl = fwd_levels[s]
            create, free = fwd_levels[source], fwd_levels[sink]
            if create < middle and free > middle:
                activations.append(e)
                for t in range(create, free + 1):
                    mem_per_timestep[t] += e.size
        
        # Get gradient nodes.
        grad_nodes = set()
        for node in self.nodes.values():
            if "_opti" in node.name:
                continue
        ...
        # Find branches need to be delayed.
        adam_factor = 3     # Need to change according to ms/ss.
        candidate = self.nodes["addcdiv__opti"]
        is_reachable = self.sink_reachability(candidate)
        to_delay = set()

        fake_peak = mem_per_timestep[middle]
        for node in grad_nodes:
            defined_bytes = node.fanout[0].size
            freed_bytes = 0
            for e in node.fanin:
                freed_bytes += e.size
            
            if defined_bytes / freed_bytes < LIMIT:
                continue
            
            fwd_level = fwd_levels[node]
            
            current_mem = mem_per_timestep[fwd_level] + adam_factor * defined_bytes
            if current_mem < fake_peak:
                continue

            fake_peak = max(adam_factor * defined_bytes, fake_peak)
            to_delay.add(node)

        # import pdb;pdb.set_trace()
        for node in to_delay:
            if node in is_reachable:
                continue
            
            t = fwd_levels[node]
            if mem_per_timestep[t] + node.fanout[0].size * adam_factor < fake_peak:
                continue
            ...
            while len(BFS) > 0:
                ...
                for e in current.fanin:
                    source = e.source
                    if source not in is_reachable:
                        target = source
                        if source not in BFS:
                            BFS.append(source)
            
            # print("[***] Adjust apply position: {} -> {}".format(candidate, target))
            self.add_edge(
                [candidate],
                [target],
                size=0,
                name=f"{target}_forced_{node}_apply_late"
            )


    # task: [bound_fwd, bound_inner_fwd, bound_inner_bwd, bound_bwd]
    def delay_apply_for_subtask(self, task, asap):
        print("Deley apply position for task: {}.".format(task))
        # import pdb;pdb.set_trace()
        apply_size = {}
        bound1 = asap[task[0]] if asap[task[0]] else -99999999
        bound2 = asap[task[1]]
        bound3 = asap[task[2]]
        bound4 = asap[task[3]] if asap[task[3]] else -99999999

        if self.grad_nodes == None or self.grad_apply == None:
            self.build_apply_grad()
        grad_nodes = self.grad_nodes
        grad_apply = self.grad_apply

        for node in self.grad_nodes:
            # We need grad_node => apply_node here.
            is_inner = False
            if bound1 < asap[node] and asap[node] < bound2:
                is_inner = True
            if bound3 < asap[node] and asap[node] < bound4:
                is_inner = True
            ...
            # grad_node => size
            apply_size[node] = size

        # Get apply start of grad node (grad_node => apply_start).
        sorted_apply_size = sorted(apply_size.items(), key=lambda x:x[1])
        delayed_grad = {}
        delayed_node = []
        num_apply, i = 3, 0
        for node, size in sorted_apply_size:
            if i > num_apply:
                break
            ...
            for e in node.fanout:
                ...
                for sink in e.sinks:
                    if "_opti" not in sink.name:
                        continue

                    is_inner = False
                    if bound1 <= asap[sink] and asap[sink] < bound2:
                        is_inner = True
                    if bound3 <= asap[sink] and asap[sink] < bound4:
                        is_inner = True

                    if not is_inner:
                        continue

                    print("Find apply start {} for grad node {}.".format(sink, node))

                    delayed_node.append(sink)
                    delayed_grad[sink] = node   # delayed node => grad node.
        
        for i in range(num_apply, -1, -1):
            node = delayed_node[i]

            # Get addcdiv_ node and the original forced_early.
            grad = delayed_grad[node]
            for apply_node in grad_apply[grad]:
                is_inner = False
                if bound1 <= asap[apply_node] and asap[apply_node] < bound2:
                    is_inner = True
                if bound3 <= asap[apply_node] and asap[apply_node] < bound4:
                    is_inner = True

                if not is_inner:
                    continue

                # if len(apply_node.fanout) != 0:
                #     import pdb;pdb.set_trace()
                
                del_edges = set()
                for e in apply_node.fanout:
                    del_edges.add(e)
                
                for e in del_edges:
                    print("Del delay apply node fanout: {}".format(e))
                    self.delete_edge(e)

        candidate = self.find_node("addcdiv__opti")
        is_reachable = self.sink_reachability(candidate)
        for i in range(num_apply, -1, -1):
            node = delayed_node[i]
            ...
            if node in is_reachable:
                ...
                # Delay the execution of grad node.
                target = node
                traverse = set()
                BFS = [node]
                while len(BFS) > 0:
                ...
                print("[***] Delay apply position: {} -> {}".format(candidate, target))
                self.add_edge(
                    [candidate],
                    [target],
                    size=0,
                ...
                # Continue to execute forced_early operation.
                # self.constrain_single_weight(apply_node)
    ...
    def _generate_candidate_sinks(self, nodes):
        sinks = set()
        for n in nodes:
            for fo in n.fanout:
                for sink in fo.sinks:
                    sinks.add(sink)
        return sinks
    ...
    def build_reachable_set(self):
        ...
        for n in self.nodes.values():
            if len(n.fanin) == 0:
                self._dfs(n, reachable_set)
        
        return reachable_set
    
    def _dfs(self, n, reachable_set):
        rset = set()
        if len(n.fanout) == 0:
            rset.add(n)
            reachable_set[n] = rset
            return rset
        
        if n in reachable_set:
            return reachable_set[n]
        
        for e in n.fanout:
            for sink in e.sinks:
                rset = rset.union(self._dfs(sink, reachable_set))
    ...
    def build_levelization(self):
        # import pdb;pdb.set_trace()
        self.fwd_level_node = defaultdict(lambda: set())
        levelization = {}
        for n in self.nodes.values():
            if n in levelization:
                self.fwd_level_node[levelization[n]].add(n)
                continue
            lvl = self._compute_level(n, levelization)
            self.fwd_level_node[lvl].add(n)
            levelization[n] = lvl
        ...
        for edge in node.fanin:
            for src in edge.sources:
                if src in levelization:
                    ...
                    # print("[BUG] repeat node: {}".format(src.name))
    ...
    def build_reverse_levelization(self):
        self.bwd_level_node = defaultdict(lambda: set())
        levelization = {}
        for n in self.nodes.values():
            if n in levelization:
                self.bwd_level_node[levelization[n]].add(n)
                continue
            lvl = self._compute_reverse_level(n, levelization)
            self.bwd_level_node[lvl].add(n)
            levelization[n] = lvl
    ...
    def _find(self, node, f):
        if f[node] == node:
            return node
        else:
            f[node] = self._find(f[node], f)
            return f[node]

    def _merge(self, node1, node2, f, rank):
        f1, f2 = self._find(node1, f), self._find(node2, f)
        if f1 == f2:
            return
    ...
    def build_set_relationship(self):
        f = {}
        rank = {}
        for node in self.nodes.values():
            f[node] = node
        ...
        for node in self.nodes.values():
            for e in node.fanout:
                for sink in e.sinks:
                    self._merge(node, sink, f, rank)

        return f

    def delete_independent_graph(self):
        set_info = {}
        largest_set, max_number = None, 0
        # import pdb;pdb.set_trace()

        f = self.build_set_relationship()
        for node in self.nodes.values():
            sid =  self._find(node, f)
            if sid in set_info:
                set_info[sid] += 1
            else:
                set_info[sid] = 1

        for sid, num in set_info.items():
            if num > max_number:
    ...
    def dump(self, filename=None, format="canon"):
        ...
        if very_large_graph:
            ...
            # dot = Digraph(format=format, engine="circo")
            # dot = Digraph(format=format, engine="twopi")
            dot = Digraph(format=format, engine="fdp")
        else:
