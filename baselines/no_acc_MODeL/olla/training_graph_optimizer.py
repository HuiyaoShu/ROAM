...
class Scheduler:
    ...
    def ComputeOptimalSchedule(
        ...
        if "vgg" in self.graph.name or "xlmr" in self.graph.name or "bert" in self.graph.name:
            num_timesteps = self.longest_path_length
        ...
        if defrag:
            ...
            for t, a in addresses.items():
                ...
                required_memory = max(required_memory, result[a] * gcd + t.size)
            required_memory += persistent_weight_size
            # print("persistent_weight_size: {} vs {}".format(persistent_weight_size, required_memory))
            # @yyp
            # required_memory -= persistent_weight_size
        else:
